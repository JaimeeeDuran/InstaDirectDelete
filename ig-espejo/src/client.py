"""Login y sesión persistente, compartida con ig-unsender.

instagrapi se importa de forma perezosa (mismo patrón que el resto del
proyecto): el dashboard arranca y se deja configurar aunque la librería no
esté instalada todavía, y los tests corren sin ella.

Por defecto `session.file` apunta a la sesión de ig-unsender. Compartir un
único login entre las dos herramientas no es un atajo: re-loguearse una y
otra vez es la señal más clara de automatización que puedes darle a
Instagram.
"""
...
log = logging.getLogger("ig-espejo.client")
...
def build_client(cfg: Config, on_2fa=None) -> Any:
    ...
    if session_path.exists():
        ...
        try:
            ...
            log.info("Sesión reutilizada desde %s", session_path)
            return cl
    ...
    log.info("Sesión nueva guardada en %s", session_path)
    return cl
